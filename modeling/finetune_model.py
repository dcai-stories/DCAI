...
from transformer_models import GPT2MemModel, GPT2LMHeadModel
    ...
    with torch.no_grad():
        ...
        for xmb1, xmb2, mem in iter_data(X1s, X2s, Ms, n_batch=n_batch_train, truncate=False, verbose=True):
            ...
            if args.use_mem:
               ...
               mem = torch.stack(mem)
               lm_logits, past = model(XMB_KG, update_mem=mem, clear_mem=True,use_pointer=args.use_pointer, use_scores=args.use_scores, mem_k=args.mem_k) 
